...
use MageTips\WindowsAzure\ServiceBus\Internal\WrapRestProxy;
use MageTips\WindowsAzure\Common\Internal\Filters\WrapFilter;
use MageTips\WindowsAzure\Common\Internal\Filters\SASFilter;
...
class ServiceBusSettings extends ServiceSettings {
    ...
    /**
     * @var \MageTips\WindowsAzure\Common\Internal\IServiceFilter
     */
    private $_filter;
    ...
    private static function createServiceBusWithWrapAuthentication(array $tokenizedSettings, $connectionString = '') {
        ...
        $endpoint = self::normalizeEndpointScheme(Utilities::tryGetValueInsensitive(
            Resources::SERVICE_BUS_ENDPOINT_NAME,
            $tokenizedSettings
        ));
    ...
    /**
     * Connection strings copied from the Azure Portal today use the "sb://"
     * scheme (the AMQP convention), but this client makes plain HTTPS REST
     * calls and Guzzle rejects any scheme other than http/https. "sb://" and
     * "https://" refer to the same namespace over different protocols, so
     * it's safe to normalize here rather than making every caller remember
     * to edit their copy-pasted connection string.
     *
     * @param string $endpoint
     * @return string
     */
    private static function normalizeEndpointScheme($endpoint) {
        return preg_replace('/^sb:\/\//i', 'https://', (string) $endpoint);
    }
    /**
